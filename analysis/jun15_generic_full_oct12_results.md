# Jun15-trained Generic Deep Fine Localization on Full Oct12 Test Set

## Setup

- Fine matcher training config: `configs/fine_pose_matcher_train_jun15_fullroutes.yaml`
- Coarse split config for hallway-oriented generic branch: `configs/coarse_retrieval_jun15_fullroutes_stride10.yaml`
...
- Note: the deep matcher is trained on `Jun. 15`; coarse retrieval uses the strongest generic checkpoint per route.
- Current generic best on `Hallway_Full` and `Hallway_Straight_CW`: keep `tracker_init`, but gate it by its own ICP quality instead of letting it compete unconditionally
- Hallway full-route configs:
  ...
  - `configs/fine_localization_oct12_hallway_straight_cw_deep_v6_guidedbev_generic_jun15coarse_trackerquality.yaml`

## Full Oct12 Summary

- Test routes: `Aisle_CCW`, `Aisle_CW`, `Hallway_Full_CW_Run_1`, `Hallway_Full_CW_Run_2`, `Hallway_Straight_CCW`, `Hallway_Straight_CW`
- Total frames: `9630`
- Weighted mean position error: `4.6724 m`
- Weighted mean yaw error: `22.6473 deg`
...
- `Hallway_Straight_CW`: `1810` frames, mean position error `3.6948 m`, median position error `0.0889 m`, mean yaw error `8.5055 deg`, below `1m` `85.58%`

## Bottleneck

- `Aisle` routes are already stable under the generic branch.
- `Hallway_Full` and `Hallway_Straight_CW` no longer need a blanket tracker shutdown; the effective generic fix is to keep `tracker_init` only when its ICP quality is strong enough.
- The dominant remaining generic error source is now `Hallway_Straight_CCW`, followed by the long-tail failures in `Hallway_Full_CW_Run_1`.
